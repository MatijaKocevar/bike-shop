...
function cellText(cell: DataTableCell): string {
...
export function DataTable({
    ...
    const visibleRows = useMemo(() => {
        ...
        columns.forEach((column, index) => {
            ...
            const byKey = new Map<Key, DataTableRow>();

            for (const row of rows) {
                if (row.key != null) byKey.set(row.key, row);
            }

            const valueOf = (row: DataTableRow) =>
                filter.type === "select"
                    ? (row.filterValues?.[filter.key] ?? "")
                    : cellText(row.cells[index] ?? { content: "" });

            const parentValueOf = (row: DataTableRow) => {
    ...
    function filterKey(filter: DataTableColumnFilter, index: number): string {
    ...
    function renderFilterControl(filter: DataTableColumnFilter, index: number) {
